# tmpx AGENTS (Design Spec)

This file is the single source of truth for the tmpx design. It defines
...
Compatibility with existing tmpx is NOT required. tmpx is a new package.

## 1. Package boundary

- New package name: `tmpx` (import path: `f4ah6o/tmpx/tmpx`).
...
- html macro sugar (tmpx/macro).
- Stateful builders (tmpx/builder).
- HTML validators (tmpx/validate).
...
for compatibility with the original tmpx. tmpx’s supported surface is
...
tmpx is in **beta**. The following contracts are frozen:
...
- Content: `text`, `raw`, `raw_unsafe`, `fragment`
...
Implementation note:
If MoonBit lacks true variadic arguments, tmpx core exposes array-based
...
## 6. Deferred: Update API (post-migration)

The immutable update surface is planned but **not** part of the current public
API. It is deferred until after the tmpx -> tmpx migration is complete.

Planned (not implemented yet): `with_attr`, `map_attrs`, `add_class`, `set_id`,
`append_children`, `prepend_children`, `map_children`, `wrap`.
